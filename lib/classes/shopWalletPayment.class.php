...
class shopWalletPayment extends waAppPayment {
    ...
    /**
     *
     * @return shopWalletPluginSettingsModel
     */
    private function model() {
        static $model;
        if (!$model) {
            $model = new shopWalletPluginSettingsModel();
        }
        return $model;
    }
    ...
    public static function getPlugin($plugin, $plugin_id = null) {
        if (!$plugin && $plugin_id) {
            ...
            if ($info['type'] != shopWalletPluginModel::TYPE_PAYMENT) {
    ...
    public static function savePlugin($plugin) {
        ...
        $model = new shopWalletPluginModel();
        if (!empty($plugin['id']) && ($id = max(0, intval($plugin['id']))) && ($row = $model->getByField(array('id' => $id, 'type' => shopWalletPluginModel::TYPE_PAYMENT)))) {
            $plugin['plugin'] = $row['plugin'];
            $model->updateById($plugin['id'], $plugin);
        } elseif (!empty($plugin['plugin'])) {
            $plugin['type'] = shopWalletPluginModel::TYPE_PAYMENT;
        ...
        if (!empty($plugin['id'])) {
            ...
            $plugins = $model->listPlugins(shopWalletPluginModel::TYPE_SHIPPING, array('all' => true,));
